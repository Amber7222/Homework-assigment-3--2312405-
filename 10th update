...
# Distinction material awnser: 
# The coefficients for “ModeVisual” indicate that the visual mode is associated with a lower likelihood of following a recommendation compared to the auditory mode.
# This is backed up by previous studies which suggest that the auditory mode tends to enhance perceptions of the communicator. 
# Studies have also shown that Auditory messages are perceived as more competent, thoughtful, and intelligent compared to written messages. 
# Nonverbal cues (e.g., tone, volume) in auditory communication impact perceptions of attitude and confidence. 
# Given the evidence, it’s possible that participants perceive auditory recommendations as more credible and trustworthy.
# This perception could influence their likelihood of following recommendations. 
# however its purely speculative as there is not much evidence from this data set for the effect of competence and intelligence on this data set so my conclusions are not fully convinced.

# Question 3 

# the beta distribution has two parameters α (alpha) and β (beta).
# To use the Beta distribution for modeling marks in PS947 (dependent varaible), you would need to transform the marks into proportions or probabilities that lie within the range [0, 1].
# This transformation could involve scaling the marks to a percentage or using some other method to represent the marks as proportions.
# Once you've transformed the dependent variable, you would then estimate the parameters α and β of the Beta distribution based on your prior beliefs about the distribution of marks. 
# These parameters could represent aspects such as the central tendency and dispersion of the marks or your beliefs about the likelihood of different ranges of marks.

# plotting an example of a beta distribution. 
# Setting the parameter values for the Beta distribution to be what is expected the marks will be. 
alpha <- 4
beta <- 3

# Created a sequence of x values that represents the possible marks. 
x <- seq(0, 1, length.out = 100)

# Defined the Beta distribution function
beta_distribution <- function(x) {
  dbeta(x, shape1 = alpha, shape2 = beta)
}

# plotted the beta distribution 
ggplot(data.frame(x = x), aes(x = x)) +
  stat_function(fun = beta_distribution, geom = "line") +
  labs(title = "Beta Distribution", x = "Marks", y = "Density")

# simulating data of possible marks to make priors 
# Set the seed for the data which ensures that the same set of random numbers is generated each time you run the code.
set.seed(123)

# Number of students in sample
n_students <- 100

# Defined parameters according to the model requirments in the slides
mean_mark <- 67
sd_mark <- 5

# Generate random coursework marks
coursework_marks <- rnorm(n_students, mean = mean_mark, sd = sd_mark)

# coursework marks should be between 0 and 100 this could makes sure its within this range.
coursework_marks <- pmax(pmin(coursework_marks, 100), 0)

# Print the coursework marks
print(coursework_marks)
